...
const App = ((UI_Ctrl) => {
  ...
  const languageChanged = (event) => {
    ...
    if (event.target.value === 'English') {
      ...
      UI_Ctrl.paintUI(frontMatter, translations, 'default');
    } else {
      UI_Ctrl.paintUI(frontMatter, translations, 'translations');

      // there are characters assoicated with the translation
      if (frontMatter[0].characters) {
        UI_Ctrl.paintUI(frontMatter, characters, 'characters');
      }
    }
  ...
  return {
    init: () => {
      ...
      http.get('./assets/data/localization.json')
        .then((response) => { 
